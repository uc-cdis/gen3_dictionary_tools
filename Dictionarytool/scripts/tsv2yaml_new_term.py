...
from ruamel_yaml.scalarstring import FoldedScalarString as fss
from ruamel_yaml.scalarstring import DoubleQuotedScalarString as dbl_quote
...
def validate_name(string, name_type, node_name):
    """
    ...
    if name_type == 'node':
        if re.search('''[^a-zA-Z]''', string[0]):
            sys.exit('First character can not be number in node name {0}'.format(string))

        match = re.search('''[^a-zA-Z_0-9]''', string)

        if match:
            sys.exit('Illegal character {0} found in node name {1}. Only lowercase letters and underscore allowed.'.format(match.string, string))

    elif name_type == 'property':
        if re.search('''[^a-zA-Z]''', string[0]):
            sys.exit('First character can not be number in variable name {0} in node {1}'.format(string, node_name))
...
if __name__ == '__main__':
    ...
    if extension == 'xlsx':
        if terms_file and not(terms_schema):
            sys.exit('ERROR: Terms {0} file not found in the directory {1}, only provide the directory path to file without file name in -i option. exiting the program'.format(extension, in_dir))

        if not(terms_file) and not(nodes_schema):
            sys.exit('ERROR: nodes_schema {0} file not found in the directory {1}, only provide the directory path to file without file name in -i option. exiting the program'.format(extension, in_dir))
        ...
        if not(terms_file) and (not(nodes_in_file) or not(var_in_file) or not(enum_in_file)):
            sys.exit('ERROR: one or more files (Nodes, Variable & Enum) with {0} extension not found in the directory {1}, only provide the directory path to file without file name in -i option. exiting the program'.format(extension, in_dir))
